...
describe('applyFinalRetaliation', () => {
  it('does nothing when no leader has just died', () => {
    const s = initialState({ cast: ['chump', 'carnage', 'burnem'], difficulty: 'normal', seed: 'x' });
  ...
  it('cascades — when death kills a survivor who also has stock, that survivor fires FR too', () => {
    ...
    // Chump then fires its own FR at burnem → 2 FinalRetaliationTriggered events.
    const s = initialState({ cast: ['chump', 'carnage', 'burnem'], difficulty: 'normal', seed: 'fr-cascade-5' });
    s.leaders.carnage.stockpile.missiles = 2;
    s.leaders.carnage.stockpile.warheadsLarge = 2;
    s.leaders.carnage.alive = false;
    s.leaders.carnage.population = 0;
    s.leaders.carnage.grudge = { chump: 100, burnem: 0 }; // all missiles → chump
    ...
    s.leaders.burnem.population = 100; // survives chump's FR
    s.leaders.burnem.deployedShields = 0;
    const r = applyFinalRetaliation(s, ['carnage']);
  ...
  it('picks FR targets weighted by the dying leader\'s grudge map (when non-empty)', () => {
    // 3-leader setup: dying leader (carnage) has heavy grudge against burnem.
    // FR should fire mostly at burnem, not chump.
    const s = initialState({ cast: ['chump', 'carnage', 'burnem'], difficulty: 'normal', seed: 'fr-grudge' });
    s.leaders.carnage.stockpile.missiles = 8;
    s.leaders.carnage.stockpile.warheadsSmall = 8;
    s.leaders.carnage.alive = false;
    s.leaders.carnage.population = 0;
    s.leaders.carnage.grudge = { burnem: 100, chump: 0 };
    // Both targets vulnerable, no defences.
    s.leaders.chump.stockpile.shields = 0;
    s.leaders.burnem.stockpile.shields = 0;
    const r = applyFinalRetaliation(s, ['carnage']);
    // All 8 launches should target burnem (weight 100 vs 0 → burnem every time).
    const launchedAtBurnem = r.events.filter(
      (e) => e.kind === 'MissileLaunched' && e.to === 'burnem',
    ).length;
    const launchedAtChump = r.events.filter(
      (e) => e.kind === 'MissileLaunched' && e.to === 'chump',
    ).length;
    expect(launchedAtBurnem).toBe(8);
    expect(launchedAtChump).toBe(0);
  });

  it('never picks a zero-weight survivor (boundary: strict > not >= in cumulative draw)', () => {
    // weights=[0, 100]: chump grudge=0, burnem grudge=100.
    ...
    for (let i = 0; i < 30; i++) {
      const s = initialState({ cast: ['chump', 'carnage', 'burnem'], difficulty: 'normal', seed: `fr-boundary-${i}` });
      s.leaders.carnage.stockpile.missiles = 4;
      s.leaders.carnage.stockpile.warheadsSmall = 4;
      s.leaders.carnage.alive = false;
      s.leaders.carnage.population = 0;
      s.leaders.carnage.grudge = { chump: 0, burnem: 100 };
      s.leaders.chump.stockpile.shields = 0;
      s.leaders.burnem.stockpile.shields = 0;
      const r = applyFinalRetaliation(s, ['carnage']);
      const launchedAtChump = r.events.filter(
  ...
  it('falls back to uniform random when grudge is empty (preserves P1 behaviour)', () => {
    const s = initialState({ cast: ['chump', 'carnage', 'burnem'], difficulty: 'normal', seed: 'fr-uniform' });
    s.leaders.carnage.stockpile.missiles = 8;
    s.leaders.carnage.stockpile.warheadsSmall = 8;
    s.leaders.carnage.alive = false;
    s.leaders.carnage.population = 0;
    ...
    expect(launchedAtBurnem + launchedAtChump).toBe(8);
    expect(launchedAtBurnem).toBeGreaterThan(0);
