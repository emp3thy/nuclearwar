...
describe('checkOutcome', () => {
  const base = initialState({ cast: ['chump', 'carnage', 'burnem'], difficulty: 'normal', seed: 'x' });

  it('returns null while multiple leaders are alive', () => {
    expect(checkOutcome(base, { chump: 33, carnage: 25, burnem: 25 })).toBeNull();
  });
  ...
  it('returns survivor when exactly one leader has pop > 0', () => {
    ...
    s.leaders.burnem.population = 0;
    s.leaders.burnem.alive = false;
    expect(checkOutcome(s, { chump: 33, carnage: 25, burnem: 25 })).toEqual({
      type: 'survivor',
      winner: 'chump',
    });
  });
  ...
  it('returns pyrrhic when every leader died this round', () => {
    const s = structuredClone(base);
    for (const id of ['chump', 'carnage', 'burnem'] as const) {
  ...
  it('returns apocalypse when nobody had population entering the round', () => {
    ...
    expect(checkOutcome(s, { chump: 0, carnage: 0, burnem: 0 })).toEqual({ type: 'apocalypse' });
  });
